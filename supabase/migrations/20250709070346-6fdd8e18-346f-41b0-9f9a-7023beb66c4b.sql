...
-- Phase 1: Fix deposit requests visibility and real-time updates

-- Ensure admin_auth_sessions table has proper foreign key linking
ALTER TABLE public.admin_auth_sessions 
ADD CONSTRAINT admin_auth_sessions_user_id_fkey 
FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;

-- Enhanced RLS policy for deposit_requests to work with admin session system
...
-- Enhanced approve_deposit_request function with better error handling
...
-- Function to get deposit request statistics for admin dashboard
...
DECLARE
  ...
  v_today_approved_count int;
  v_today_approved_amount numeric;
BEGIN
  ...
  SELECT 
    COUNT(*), 
    COALESCE(SUM(amount), 0)
  INTO v_today_approved_count, v_today_approved_amount
  FROM public.deposit_requests 
  ...
  RETURN json_build_object(
    ...
    'today_approved_count', v_today_approved_count,
    'today_approved_amount', v_today_approved_amount
  );
END;
$$;
