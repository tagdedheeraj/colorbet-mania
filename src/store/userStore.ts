...
import { User, UserProfile } from '../types/game';
...
const getStoredUsers = (): Record<string, { username: string; password: string; balance: number; profile?: UserProfile; transactions?: any[]; referralCode?: string; referredBy?: string }> => {
  const stored = localStorage.getItem(USERS_STORAGE_KEY);
  return stored ? JSON.parse(stored) : {};
};

// Helper function to save users to local storage
const saveUsers = (users: Record<string, { username: string; password: string; balance: number; profile?: UserProfile; transactions?: any[]; referralCode?: string; referredBy?: string }>) => {
...
export { getStoredUsers, saveUsers, getCurrentUser, saveCurrentUser };
